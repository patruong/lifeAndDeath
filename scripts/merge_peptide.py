...
for i in range(180):
    ...
    df_PEP_treshold = df[df["PEP"] < 0.01]
    df_PEP_MissedCleaveges_treshold = df_PEP_treshold[df_PEP_treshold["Missed_Cleaveges"] < 2]
    #del df_PEP_treshold
    #df = df_PEP_MissedCleaveges_treshold
    #del df_PEP_MissedCleaveges_treshold
    #df = df.drop(["Missed_Cleaveges", "Charges", "Score", "PEP", "Gene_names"], axis = 1)
    res = res.append(df)
    proc_time = time.time()
